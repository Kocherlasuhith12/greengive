...
export default function CharityPage() {
  ...
  async function save() {
    ...
    const { error } = await supabase
      .from('subscriptions')
      .update({ charity_id: selectedId || null, charity_percentage: percentage })
  ...
  if (loading) return (
    <div className="pt-14 lg:pt-0 space-y-4">
      {[...Array(4)].map((_, i) => <div key={i} className="h-24 bg-dark-700 rounded-2xl animate-pulse" />)}
    </div>
  )
  ...
  return (
    <div className="space-y-6 pt-14 lg:pt-0">
      <div>
        <h1 className="text-2xl font-display font-bold text-white">My Charity</h1>
        <p className="text-gray-400 text-sm mt-1">Choose who benefits from your subscription</p>
      </div>
      ...
      {subscription && (
        <div className="p-5 rounded-2xl bg-pink-900/10 border border-pink-800/30">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm font-medium text-white">Your contribution this {subscription.plan === 'monthly' ? 'month' : 'year'}</span>
          </div>
          <div className="flex items-baseline gap-2">
            <span className="text-3xl font-display font-bold text-pink-400">
              {formatCurrency(monthlyContribution)}
            </span>
            <span className="text-gray-500 text-sm">({percentage}% of {formatCurrency(subscription.amount_pence)})</span>
          </div>
          {selected && <p className="text-xs text-gray-500 mt-1">→ going to {selected.name}</p>}
        </div>
      )}

      {/* Charity percentage slider */}
      <div className="card p-6">
        <h2 className="text-base font-semibold text-white mb-4">Contribution percentage</h2>
        ...
        <div className="flex justify-between text-xs text-gray-600 mt-1">
          <span>Minimum 10%</span><span>Maximum 50%</span>
        </div>
      </div>

      {/* Charity grid */}
      <div>
        <h2 className="text-base font-semibold text-white mb-4">Choose your charity</h2>
          ...
          {charities.map(charity => (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                ...
                {selectedId === charity.id && (
                  <CheckCircle size={18} className="text-pink-400 shrink-0 mt-0.5" />
                )}
              </div>
              <div className="mt-3">
                ...
                {charity.description && (
                  <p className="text-xs text-gray-500 mt-1 line-clamp-2">{charity.description}</p>
                )}
